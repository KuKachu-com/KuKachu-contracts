...
contract KuKachuBscTest is AccessControl, BEP20, ReentrancyGuard {
    ...
    uint256 private _buyBackTokenBalance = 0;
    uint256 private _buyBackBnbBalance = 0;

    /**
     * @dev Trick bot with different name and symbol when deploy
    */
    ...
    event TokensBurned(uint256 bnbSpent, uint256 tokensBurned);
    ...
    function fallbackBNB() external onlyRole(ADMIN_ROLE){
        (bool sent,) = address(msg.sender).call{value : _thisAddress.balance}("");
    ...
    function tokenBalanceOfBuyBack() public view returns (uint256) {
        return _buyBackTokenBalance;
    }
    ...
    function _swapAndAddLp() private {
        ...
        if (_isSell) {
            ...
            tokenAmountToBeSwapped = initialBalance.sub(lpTokensToAdd).sub(tokenBalanceOfBuyBack());
    ...
    function _flipAndBurnTokens(uint256 bnbAmount) private {
        ...
        emit TokensBurned(bnbAmount, tReceived);
        _midSwap = false;
    }
    ...
    function _setCooldownOf(address recipient) internal {
        _cooldownOf[recipient] = block.timestamp + _COOLDOWN;
    }
    ...
    function _beforeClaim() internal virtual {
        require(_contractIsLaunched, "KuKachu: contract is not launched");
        require(_thisAddress.balance > 0, "KuKachu, contract zero BNB balance");
        require(bnbBalanceOf(msg.sender) > 0, "KuKachu, BNB balanceOf is zero");
